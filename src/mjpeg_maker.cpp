...
void catch_signal(int sig)
{
	...
	ACE_DEBUG ((LM_DEBUG,
	              "(%P|%t) k value is %d \n ", sig));
	return;
	if (sig != CTRL_C && sig != CTRL_Z)		// CTRL_Z will forcefully quit the system. should not be used in normal situation
		return;								// using CTRL_C to quit system is okay but not encouraged

	if (sig == CTRL_Z)
		;//stop = 1;

	//sleep(1);
	//pSharedMemStruct->Command = QUIT;
	//run = 0;
}
